...
from tracking.models.siammask_tracker.features import MultiStageFeature
from tracking.models.siammask_tracker.mask import Mask
from tracking.models.siammask_tracker.resnet import resnet50
from tracking.models.siammask_tracker.rpn import DepthCorr, RPN
from tracking.models.siammask_tracker.siammask_sharp import SiamMask
...
class Custom(SiamMask):
    def __init__(self, pretrain=False, **kwargs):
        ...
        self.total_stride = 8  # spatial downsampling ratio of the model
        self.base_size = 8  # reduced size due to cropping the template feature maps
    ...
    def query(self, template, search, enable_mask=False):
        rpn_pred_cls, rpn_pred_loc = self.rpn(template, search)
        if enable_mask:
            self.corr_feature = self.mask_model.mask.forward_corr(template, search)
            pred_mask = self.mask_model.mask.head(self.corr_feature)
        else:
            pred_mask = None
        return rpn_pred_cls, rpn_pred_loc, pred_mask
