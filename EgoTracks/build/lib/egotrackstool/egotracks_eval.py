...
class _EGO4DLTTrackingDataset(BaseDataset):
    ...
    def __init__(self, annotation_path, data_dir="/checkpoint/haotang/tracking/data/ego4d/clips_frames/", split=None):
        super().__init__()
...
def load_submission(submission_dir:str):
    ...
    for f in tqdm(files, total=len(files)):
        ...
        with open(os.path.join(submission_dir, f), "r") as f:
...
def compute_competition_metrics(submission, annotation):
    ...
    for seq in tqdm(annotation.sequences, total=len(annotation.sequences)):
        ...
        pred_bbox_dict = {r[0]: {
            "bbox": r[1:-1], "score": r[-1]
        } for r in submission[seq.name]}
        ...
        for fn in range(total_frame_numbers):
            if fn not in pred_bbox_dict:
                raise f"No prediction for frame {fn} in {seq}."
